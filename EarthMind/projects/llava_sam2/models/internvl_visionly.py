...
def cross_attn(Q, K, V, tau=1.0):
    # Q:(B, M, D)  K,V:(B, N, D) -> out:(B, M, D)
    attn = torch.matmul(Q, K.transpose(-2, -1)) / (sqrt(2048) * tau)  # (B, M, N)
...
def extract_feature_with_projector_toggle(self, pixel_values, use_projector=True):
...
class InternVL_Slowfast(InternVL_V1_5):
    ...
    def __init__(self,
        ...
        self.hca_queries = nn.Parameter(torch.randn(256, 2048))  # M=256, D=hidden
        self.hca_tau = 1.0  

        # self.query_token = nn.Parameter(torch.randn(1, 2048))  # D = 2048
        # self.attn_scale = 2048 ** -0.5  # dot product attention scale
        ...
        import types
        self.model.extract_feature = types.MethodType(extract_feature_with_projector_toggle, self.model)
    ...
    def forward(self, data, data_samples=None, mode='loss', fast_token_idx=None):
        ...
        # Load SAR as main visual input (default)
        ...
        # Load RGB image as auxiliary input (for contrastive learning etc.)
        if 'rgb_pixel_values' in data:
            rgb_values = data['rgb_pixel_values']
            if type(rgb_values) is list or rgb_values.ndim == 5:
                if type(rgb_values) is list:
                    rgb_values = [
                        x.unsqueeze(0) if x.ndim == 3 else x for x in rgb_values
                    ]
                rgb_concat_images = torch.cat(
                    [image.to(self.model.vision_model.dtype) for image in rgb_values], dim=0)
            else:
             
                raise NotImplementedError()
        else:
            rgb_concat_images = None
        ...
        # print("1111111",concat_images.shape,rgb_concat_images.shape)
        
        if rgb_concat_images is not None:
            outputs,image_tokens_pair,feature_pairs,image_id= self._llm_forward(
                input_ids=input_ids,
                position_ids=position_ids,
                attention_mask=attention_mask,
                image_flags=image_flags,
                pixel_values=concat_images,  # SAR
                labels=labels,
                use_cache=use_cache,
                output_hidden_states=True,
                ...
                rgb_pixel_values=rgb_concat_images,  # SAR
            )


            return outputs,image_tokens_pair,feature_pairs,image_id
        
        else:
            outputs = self._single_llm_forward(
            input_ids=input_ids,
            position_ids=position_ids,
            attention_mask=attention_mask,
            image_flags=image_flags,
    ...
    def _llm_forward(
        ...
        image_token_mask = (input_ids == 151667).long() 
        # print("image",image_token_mask.shape)
        ...
        if fast_pixel_values is not None:
            n_fast_images = fast_pixel_values.shape[0]
            whole_pixel_values = torch.cat([fast_pixel_values, pixel_values], dim=0)
            vit_embeds = self.extract_feature(whole_pixel_values)
            vit_embeds = vit_embeds.to(input_embeds.dtype)  # FIXME: why vit_embeds is float16?
            fast_vit_embeds = vit_embeds[:n_fast_images]  # (n_fast_images, hw, c)
            _size = int(fast_vit_embeds.shape[1] ** 0.5)
            fast_vit_embeds = fast_vit_embeds.reshape(fast_vit_embeds.shape[0], _size, _size, fast_vit_embeds.shape[-1])
            ...
            vit_embeds = self.model.extract_feature(pixel_values, use_projector=True)
            image_features = self.model.extract_feature(pixel_values, use_projector=False)
            image_tokens=vit_embeds.clone()
            if rgb_pixel_values is not None:
                # 先提取两个模态的ViT特征
                rgb_vit_embeds = self.model.extract_feature(rgb_pixel_values, use_projector=True)
                rgb_image_tokens = rgb_vit_embeds.clone()
        ...
        input_embeds = input_embeds.reshape(B, N, C)
        # print("final",input_embeds.shape)
        # print(a)
        outputs = self.model.language_model(
            inputs_embeds=input_embeds,
            attention_mask=attention_mask,
        ...
        last_hidden = outputs.hidden_states[-1]   # shape: (B * N, C
        # input_ids: [B, N] -> flatten
        selected = (input_ids == self.model.img_context_token_id)  # shape: [B, N] = [2, 1456]
        selected_flat = selected.view(-1)                          # shape: [2*1456] = [2912]

        # last_hidden: [B, N, C] -> flatten
        last_hidden_flat = last_hidden.view(-1, last_hidden.shape[-1])  # shape: [2*1456, 2048]

        # 使用掩码选中 image token 的 LLM 输出表示
        llm_image_token_output = last_hidden_flat[selected_flat]  # shape: [n_img_token, C]
        B_img, N_patch, _ = image_features.shape
        llm_image_token_output = llm_image_token_output.view(B_img, -1, 2048)
        seman_fea1 = llm_image_token_output[:, :256, :]  # [10, 256, 2048]
        seman_fea2 = llm_image_token_output[:, 256:, :]  # [10, 256, 2048]
        

        token_pairs=(image_tokens,rgb_image_tokens)
        feature_pairs=(seman_fea1,seman_fea2)
        return CausalLMOutputWithPast(
            loss=loss,
            logits=logits,
            past_key_values=outputs.past_key_values,
            hidden_states=outputs.hidden_states,
            attentions=outputs.attentions,
        ),token_pairs,feature_pairs, self.model.img_context_token_id
