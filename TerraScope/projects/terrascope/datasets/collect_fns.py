...
#     has_region_info = any(inst.get('region_info') is not None for inst in instances)
...
#         if has_region_info:
#             if 'region_info' in example.keys() and example['region_info'] is not None:
#                 region_infos.append(example['region_info'])
...
#     if has_region_info:
#         data_dict['region_info'] = region_infos
...
# from your_pkg.distributed import get_sequence_parallel_world_size, pad_for_sequence_parallel
...
def video_lisa_collate_fn(instances: Sequence[Dict],
    ...
    has_gt_answers = any(
        (inst.get('gt_answer') is not None) or
        (inst.get('conversations') is not None)
    ...
    if use_varlen_attn:
        position_ids, cumulative_len = [], []
        assert len(instances) == 1, (
            f'If utilizing varlen attention, the batch size should be set to 1, but got {len(instances)}')
        assert not has_image, (
            'Currently, it is not configured to accommodate the use of varlen Attention in multimodal training'
        )

    if has_image:
        pixel_values, frames_per_batch, image_grid_thw = [], [], []
    if has_grounding_image:
        grounding_pixel_values = []
    if has_mask:
        object_masks = []
    ...
    gt_answers = [] if has_gt_answers else None

    def _extract_answer_letter_from_conversations(conv_list):
        """
          - 'The answer is D.'
          - 'Answer: B' / ' C'
          - 
        """
        if not isinstance(conv_list, (list, tuple)):
            return None
        letter = None
        patts = [
            r'(?:the\s+answer\s+is|answer\s*[:]|\s*)\s*([A-D])\b',
            r'\s*([A-D])\b'
        ]
        for turn in conv_list:
            if not isinstance(turn, dict):
                continue
            if turn.get('from', '').lower() not in ('gpt', 'assistant', 'system'):
                continue
            text = str(turn.get('value', ''))
            for p in patts:
                m_iter = list(re.finditer(p, text, flags=re.IGNORECASE))
                if m_iter:
                    letter = m_iter[-1].group(1).upper()
        return letter
    ...
    for example in instances:
        ...
        if has_image:
            ...
            if has_pe:
                image_grid_thw.append(example['image_grid_thw'])
            if has_vp:
                if 'vp_overall_mask' in example and example['vp_overall_mask'] is not None:
                    vp_overall_mask.append(example['vp_overall_mask'])
                else:
                    vp_overall_mask.append(torch.Tensor([False] * len(pixel_values[-1])))

        if has_fast_image and example.get('fast_pixel_values') is not None:
            fast_pixel_values.append(example['fast_pixel_values'])

        if has_fast_exists and example.get('fast_exists') is not None:
            fast_exists.append(example['fast_exists'])

        if has_grounding_image and 'g_pixel_values' in example:
            if isinstance(example['g_pixel_values'], list):
                grounding_pixel_values += example['g_pixel_values']
                frames_per_batch.append(len(example['g_pixel_values']))
            else:
        ...
        if has_mask and example.get('masks') is not None:
            if isinstance(example['masks'], list):
                if isinstance(example['masks'][0], np.ndarray):
                    _masks = np.stack(example['masks'], axis=0)
                    _masks = torch.from_numpy(_masks)
        ...
        if has_bboxes and example.get('bboxes') is not None:
            object_bboxes.append(example['bboxes'])

        if has_points and example.get('points') is not None:
            prompt_points.append(example['points'])

        if has_region_info and example.get('region_info') is not None:
            region_infos.append(example['region_info'])

        if has_prompt_mask and 'prompt_masks' in example:
            prompt_masks.append(example['prompt_masks'])

        if gt_answers is not None:
            if example.get('gt_answer') is not None:
                letter = str(example['gt_answer']).strip().upper()
                gt_answers.append(letter if letter in ('A', 'B', 'C', 'D') else None)
            else:
                letter = _extract_answer_letter_from_conversations(example.get('conversations'))
                gt_answers.append(letter if letter in ('A', 'B', 'C', 'D') else None)
    ...
    if use_varlen_attn:
        max_seqlen = (cumulative_len[0][1:] - cumulative_len[0][:-1]).max().item()
        data_dict = {
            'input_ids': input_ids,
            'cumulative_len': cumulative_len,
            'position_ids': position_ids,
    ...
    if has_fast_image and len(fast_pixel_values) > 0:
        if all(x.shape == fast_pixel_values[0].shape for x in fast_pixel_values):
            fast_pixel_values = torch.stack(fast_pixel_values, dim=0)
        data_dict['fast_pixel_values'] = fast_pixel_values
    ...
    if gt_answers is not None:
        data_dict['gt_answers'] = gt_answers
