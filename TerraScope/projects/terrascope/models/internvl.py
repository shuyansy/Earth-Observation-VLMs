...
class InternVL_Slowfast(InternVL_V1_5):
    ...
    def __init__(self,
        ...
        region_context_token_id = tokenizer.convert_tokens_to_ids('<REGION>')
        self.model.region_context_token_id = region_context_token_id
    ...
    def forward(self, data, data_samples=None, mode='loss', fast_token_idx=None):
        ...
        if "region_info" in data.keys():
            ...
            outputs = self._llm_forward_naive(
                input_ids=input_ids,
                position_ids=position_ids,
                attention_mask=attention_mask,
                image_flags=image_flags,
    ...
    def _llm_forward(
        ...
        # print("******* Original vit_embeds.shape:", vit_embeds.shape)  # (8, 256, 2048)
        # print("@@@@", region_info)
        
        vit_embeds = vit_embeds[image_flags == 1]
        # print("*******vit_embeds.shape:", vit_embeds.shape)  # (8, 256, 2048)
        vit_batch_size = pixel_values.shape[0]

        vit_embeds_3d = vit_embeds  # (num_patches, 256, C)
        num_patches = vit_embeds_3d.shape[0]
        tokens_per_patch = vit_embeds_3d.shape[1]
        
        vit_embeds_flat = vit_embeds_3d.reshape(-1, vit_embeds_3d.shape[-1])  # (num_patches * 256, C)
        ...
        try:
            input_embeds[selected] = vit_embeds_flat.reshape(-1, C)
        except Exception as e:
            vit_embeds_flat = vit_embeds_flat.reshape(-1, C)
            print(f'warning: {e}, input_embeds[selected].shape='
                    f'{input_embeds[selected].shape}, '
                    f'vit_embeds_flat.shape={vit_embeds_flat.shape}')
            n_token = selected.sum()
            if n_token > len(vit_embeds_flat):
                print(f"Wrong !!! {n_token} image tokens in text but only {len(vit_embeds_flat)} vit embeds !!!")
                expand_ratio = n_token // len(vit_embeds_flat) + 1
                vit_embeds_flat = torch.cat([vit_embeds_flat] * expand_ratio, dim=0)

            input_embeds[selected] = vit_embeds_flat[:n_token]
        
        if region_info is not None and len(region_info) > 0:
            
            # print(f"\n🔍 Processing region tokens:")
            # print(f"   vit_embeds_flat.shape: {vit_embeds_flat.shape}")
            # print(f"   num_patches: {num_patches}, tokens_per_patch: {tokens_per_patch}")
            # print(f"   Total visual tokens: {num_patches * tokens_per_patch}")
            ...
            for batch_idx in range(B):
                ...
                sample_input_ids = input_ids[sample_start_idx:sample_end_idx]
                
                region_positions = (sample_input_ids == self.model.region_context_token_id).nonzero(as_tuple=True)[0]
                
                if len(region_positions) == 0:
                    print(f"⚠️  Warning: No <REGION> tokens found for batch {batch_idx}")
                    continue
                
                # print(f"\n   📦 Batch {batch_idx}: {len(region_positions)} <REGION> tokens found")
                
                region_idx = 0
                
                # print("******",sample_region_info)
                # For query_select fusion: extract question text embeddings once per sample
                # Question tokens = input tokens that are not <IMG_CONTEXT> and not <REGION>
                _query_embeds_cache = None

                def _get_query_embeds():
                    nonlocal _query_embeds_cache
                    if _query_embeds_cache is not None:
                        return _query_embeds_cache
                    is_img = (sample_input_ids == self.model.img_context_token_id)
                    is_region = (sample_input_ids == self.model.region_context_token_id)
                    is_text = ~is_img & ~is_region
                    text_positions = is_text.nonzero(as_tuple=True)[0]
                    if len(text_positions) == 0:
                        # fallback: use all input embeds
                        _query_embeds_cache = input_embeds[sample_start_idx:sample_end_idx]
                    else:
                        _query_embeds_cache = input_embeds[sample_start_idx + text_positions]  # (L, C)
                    return _query_embeds_cache

                for obj_idx, obj_info in enumerate(sample_region_info):
                    num_tokens = obj_info['num_tokens']
                    fusion_mode = obj_info.get('fusion_mode', None)

                    if region_idx + num_tokens > len(region_positions):
                        print(f"⚠️  Warning: Not enough <REGION> tokens for object {obj_idx}")
                        break

                    try:
                        if fusion_mode == 'query_select':
                            # Text-guided token-level modality selection (SAR+RGB)
                            rgb_indices = obj_info['rgb_token_indices']
                            sar_indices = obj_info['sar_token_indices']

                            max_rgb = max(rgb_indices)
                            max_sar = max(sar_indices)
                            if max_rgb >= vit_embeds_flat.shape[0] or max_sar >= vit_embeds_flat.shape[0]:
                                print(f"❌ Error: index out of range for object {obj_idx}")
                                continue

                            v_opt = vit_embeds_flat[rgb_indices]   # (K, C)
                            v_sar = vit_embeds_flat[sar_indices]   # (K, C)

                            q = _get_query_embeds()  # (L, C)
                            D = q.shape[-1]

                            # Cross-attention scores: (K, L) = V @ Q^T / sqrt(D)
                            # Then softmax over visual dim (each text token attends to visual tokens)
                            # beta_j = (1/L) * sum_l softmax(V @ Q^T / sqrt(D))_{j,l}
                            attn_opt = torch.matmul(v_opt, q.t()) / (D ** 0.5)  # (K, L)
                            attn_sar = torch.matmul(v_sar, q.t()) / (D ** 0.5)  # (K, L)

                            # Softmax over visual token dim (dim=0) for each text token
                            attn_opt = torch.softmax(attn_opt, dim=0)  # (K, L)
                            attn_sar = torch.softmax(attn_sar, dim=0)  # (K, L)

                            # Aggregate across text tokens: mean over L
                            beta_opt = attn_opt.mean(dim=1)  # (K,)
                            beta_sar = attn_sar.mean(dim=1)  # (K,)

                            # Per-position selection
                            select_sar = beta_sar > beta_opt  # (K,)
                            region_features = torch.where(
                                select_sar.unsqueeze(-1), v_sar, v_opt
                            )  # (K, C)
                        else:
                            # Original path: direct index lookup
                            visual_indices = obj_info['visual_token_indices']
                            max_index = max(visual_indices)
                            if max_index >= vit_embeds_flat.shape[0]:
                                print(f"❌ Error: visual_index {max_index} >= vit_embeds size {vit_embeds_flat.shape[0]}")
                                continue
                            region_features = vit_embeds_flat[visual_indices]  # (num_tokens, C)

                        # Replace <REGION> positions with selected features
                ...
                if region_idx != len(region_positions):
                    print(f"⚠️  Warning: Mismatch in <REGION> count for batch {batch_idx}. "
                          f"Expected {len(region_positions)}, processed {region_idx}")
    ...
    #     print("*******",vit_embeds.shape,input_embeds.shape)    #******* torch.Size([7, 256, 2048]) torch.Size([1, 2153, 2048])
    #     print("@@@@",region_info)
    #     vit_embeds = vit_embeds[image_flags == 1]
    #     vit_batch_size = pixel_values.shape[0]
