...
class Multi_RefCocoDataset(BaseDataset):
    """RefCOCO dataset.
    ...
    def load_data_list(self) -> List[dict]:
        ...
        rgb_prefix = self.data_prefix['rgb_path']
        sar_prefix = self.data_prefix['sar_path']
        ...
        rgb_backend = mmengine.fileio.get_file_backend(rgb_prefix)
        sar_backend = mmengine.fileio.get_file_backend(sar_prefix)
        ...
        for image in images:
            ...
            rgb_path = rgb_backend.join_path(rgb_prefix, image['file_name'])
            sar_path = sar_backend.join_path(sar_prefix, image['file_name'].replace('.jpg', '.png'))

            data_info = {
                'rgb_img_path': rgb_path,
                'sar_img_path': sar_path,
                'img_id': img_id,
                'instances': instances,
                'text': sentences
            }
