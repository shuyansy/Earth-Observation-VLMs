...
from third_parts_multi.mmdet.datasets.refcoco import Multi_RefCocoDataset
...
class Multi_ReferSegmDataset(Multi_RefCocoDataset):
    os.environ['TOKENIZERS_PARALLELISM'] = 'true'
    IMG_CONTEXT_TOKEN = '<IMG_CONTEXT>'
    IMG_START_TOKEN = '<img>'
    IMG_END_TOKEN = '</img>'
    ...
    def _parse_annotations(self, ann_info):
        ...
        rgb_image_path = ann_info['rgb_img_path']
        sar_image_path = ann_info['sar_img_path']
        ...
        ann_info.update({
            'masks': masks,
            'conversations': conversation,
            'image': (rgb_image_path,sar_image_path)
        })
        return ann_info
    ...
    def prepare_data(self, index):
        ...
        if data_dict.get('image', None) is not None:
            rgb_image_path, sar_image_path = data_dict['image']
            try:
                rgb_image = Image.open(rgb_image_path).convert('RGB')
                sar_image = Image.open(sar_image_path).convert('RGB')
            except Exception as e:
            ...
            if hasattr(self, 'extra_image_processor'):
                g_image = np.array(sar_image)
                g_image = self.extra_image_processor.apply_image(g_image)
                g_pixel_values = torch.from_numpy(g_image).permute(2, 0, 1).contiguous()
                out_data_dict['g_pixel_values'] = g_pixel_values

                rgb_g_image = np.array(rgb_image)
                rgb_g_image = self.extra_image_processor.apply_image(rgb_g_image)
                rgb_g_pixel_values = torch.from_numpy(rgb_g_image).permute(2, 0, 1).contiguous()
                out_data_dict['rgb_g_pixel_values'] = rgb_g_pixel_values
            ...
            if self.preprocessor is not None:
                ...
                sar_tensor = torch.stack([self.transformer(im) for im in sar_images])
                rgb_tensor = torch.stack([self.transformer(im) for im in rgb_images])
                out_data_dict['pixel_values'] = sar_tensor
                out_data_dict['rgb_pixel_values'] = rgb_tensor
                num_image_tokens = sar_tensor.shape[0] * self.patch_token
            ...
            out_data_dict.update(token_dict)
            out_data_dict['pixel_values'] = torch.zeros(1, 3, self.image_size, self.image_size)
            out_data_dict['rgb_pixel_values'] = torch.zeros(1, 3, self.image_size, self.image_size)
...
if __name__ == '__main__':
    ...
    from third_parts_multi.segment_anything.utils.transforms import ResizeLongestSide
